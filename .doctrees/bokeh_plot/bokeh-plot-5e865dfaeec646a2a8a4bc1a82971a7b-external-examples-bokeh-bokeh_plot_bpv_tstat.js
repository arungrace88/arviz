(function() {
  var fn = function() {
    
    (function(root) {
      function now() {
        return new Date();
      }
    
      var force = false;
    
      if (typeof root._bokeh_onload_callbacks === "undefined" || force === true) {
        root._bokeh_onload_callbacks = [];
        root._bokeh_is_loading = undefined;
      }
    
      
      
    
      var element = document.getElementById("26914ec4-4351-491c-841e-d43858f81d35");
        if (element == null) {
          console.warn("Bokeh: autoload.js configured with elementid '26914ec4-4351-491c-841e-d43858f81d35' but no matching script tag was found.")
        }
      
    
      function run_callbacks() {
        try {
          root._bokeh_onload_callbacks.forEach(function(callback) {
            if (callback != null)
              callback();
          });
        } finally {
          delete root._bokeh_onload_callbacks
        }
        console.debug("Bokeh: all callbacks have finished");
      }
    
      function load_libs(css_urls, js_urls, callback) {
        if (css_urls == null) css_urls = [];
        if (js_urls == null) js_urls = [];
    
        root._bokeh_onload_callbacks.push(callback);
        if (root._bokeh_is_loading > 0) {
          console.debug("Bokeh: BokehJS is being loaded, scheduling callback at", now());
          return null;
        }
        if (js_urls == null || js_urls.length === 0) {
          run_callbacks();
          return null;
        }
        console.debug("Bokeh: BokehJS not loaded, scheduling load and callback at", now());
        root._bokeh_is_loading = css_urls.length + js_urls.length;
    
        function on_load() {
          root._bokeh_is_loading--;
          if (root._bokeh_is_loading === 0) {
            console.debug("Bokeh: all BokehJS libraries/stylesheets loaded");
            run_callbacks()
          }
        }
    
        function on_error() {
          console.error("failed to load " + url);
        }
    
        for (var i = 0; i < css_urls.length; i++) {
          var url = css_urls[i];
          const element = document.createElement("link");
          element.onload = on_load;
          element.onerror = on_error;
          element.rel = "stylesheet";
          element.type = "text/css";
          element.href = url;
          console.debug("Bokeh: injecting link tag for BokehJS stylesheet: ", url);
          document.body.appendChild(element);
        }
    
        const hashes = {"https://cdn.bokeh.org/bokeh/release/bokeh-2.2.3.min.js": "T2yuo9Oe71Cz/I4X9Ac5+gpEa5a8PpJCDlqKYO0CfAuEszu1JrXLl8YugMqYe3sM", "https://cdn.bokeh.org/bokeh/release/bokeh-widgets-2.2.3.min.js": "98GDGJ0kOMCUMUePhksaQ/GYgB3+NH9h996V88sh3aOiUNX3N+fLXAtry6xctSZ6", "https://cdn.bokeh.org/bokeh/release/bokeh-tables-2.2.3.min.js": "89bArO+nlbP3sgakeHjCo1JYxYR5wufVgA3IbUvDY+K7w4zyxJqssu7wVnfeKCq8"};
    
        for (var i = 0; i < js_urls.length; i++) {
          var url = js_urls[i];
          var element = document.createElement('script');
          element.onload = on_load;
          element.onerror = on_error;
          element.async = false;
          element.src = url;
          if (url in hashes) {
            element.crossOrigin = "anonymous";
            element.integrity = "sha384-" + hashes[url];
          }
          console.debug("Bokeh: injecting script tag for BokehJS library: ", url);
          document.head.appendChild(element);
        }
      };
    
      function inject_raw_css(css) {
        const element = document.createElement("style");
        element.appendChild(document.createTextNode(css));
        document.body.appendChild(element);
      }
    
      
      var js_urls = ["https://cdn.bokeh.org/bokeh/release/bokeh-2.2.3.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-widgets-2.2.3.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-tables-2.2.3.min.js"];
      var css_urls = [];
      
    
      var inline_js = [
        function(Bokeh) {
          Bokeh.set_log_level("info");
        },
        
        function(Bokeh) {
          (function() {
            var fn = function() {
              Bokeh.safely(function() {
                (function(root) {
                  function embed_document(root) {
                    
                  var docs_json = '{"d6d6ced0-0681-4e99-9a96-1bba85ce6f13":{"roots":{"references":[{"attributes":{},"id":"65574","type":"Selection"},{"attributes":{"callback":null},"id":"65545","type":"HoverTool"},{"attributes":{"source":{"id":"65557"}},"id":"65561","type":"CDSView"},{"attributes":{"fill_alpha":0.5,"fill_color":"lightgrey","level":"overlay","line_alpha":1.0,"line_color":"black","line_dash":[4,4],"line_width":2,"xs_units":"screen","ys_units":"screen"},"id":"65547","type":"PolyAnnotation"},{"attributes":{},"id":"65572","type":"BasicTickFormatter"},{"attributes":{"label":{"value":"bpv=0.53"},"renderers":[{"id":"65565"}]},"id":"65580","type":"LegendItem"},{"attributes":{"line_alpha":0.1,"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"65564","type":"Line"},{"attributes":{},"id":"65543","type":"UndoTool"},{"attributes":{"active_drag":"auto","active_inspect":"auto","active_multi":null,"active_scroll":"auto","active_tap":"auto","tools":[{"id":"65538"},{"id":"65539"},{"id":"65540"},{"id":"65541"},{"id":"65542"},{"id":"65543"},{"id":"65544"},{"id":"65545"}]},"id":"65548","type":"Toolbar"},{"attributes":{"axis":{"id":"65534"},"dimension":1,"ticker":null},"id":"65537","type":"Grid"},{"attributes":{"data_source":{"id":"65557"},"glyph":{"id":"65558"},"hover_glyph":null,"muted_glyph":null,"nonselection_glyph":{"id":"65559"},"selection_glyph":null,"view":{"id":"65561"}},"id":"65560","type":"GlyphRenderer"},{"attributes":{"data_source":{"id":"65581"},"glyph":{"id":"65582"},"hover_glyph":null,"muted_glyph":null,"nonselection_glyph":{"id":"65583"},"selection_glyph":null,"view":{"id":"65585"}},"id":"65584","type":"GlyphRenderer"},{"attributes":{"data":{},"selected":{"id":"65598"},"selection_policy":{"id":"65597"}},"id":"65581","type":"ColumnDataSource"},{"attributes":{"fill_alpha":{"value":0.1},"fill_color":{"value":"#2a2eec"},"line_alpha":{"value":0.1},"size":{"units":"screen","value":6.0},"x":{"value":-1.135840820153207},"y":{"value":0}},"id":"65583","type":"Circle"},{"attributes":{"source":{"id":"65581"}},"id":"65585","type":"CDSView"},{"attributes":{"formatter":{"id":"65570"},"ticker":{"id":"65535"}},"id":"65534","type":"LinearAxis"},{"attributes":{},"id":"65524","type":"DataRange1d"},{"attributes":{"bottom_units":"screen","fill_alpha":0.5,"fill_color":"lightgrey","left_units":"screen","level":"overlay","line_alpha":1.0,"line_color":"black","line_dash":[4,4],"line_width":2,"right_units":"screen","top_units":"screen"},"id":"65546","type":"BoxAnnotation"},{"attributes":{},"id":"65573","type":"UnionRenderers"},{"attributes":{"axis":{"id":"65530"},"ticker":null},"id":"65533","type":"Grid"},{"attributes":{"children":[[{"id":"65521"},0,0]]},"id":"65602","type":"GridBox"},{"attributes":{"data":{},"selected":{"id":"65576"},"selection_policy":{"id":"65575"}},"id":"65562","type":"ColumnDataSource"},{"attributes":{"line_alpha":0,"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"65563","type":"Line"},{"attributes":{},"id":"65535","type":"BasicTicker"},{"attributes":{},"id":"65575","type":"UnionRenderers"},{"attributes":{},"id":"65539","type":"PanTool"},{"attributes":{},"id":"65597","type":"UnionRenderers"},{"attributes":{},"id":"65598","type":"Selection"},{"attributes":{"data":{"x":{"__ndarray__":"","dtype":"float64","order":"little","shape":[512]},"y":{"__ndarray__":"","dtype":"float64","order":"little","shape":[512]}},"selected":{"id":"65574"},"selection_policy":{"id":"65573"}},"id":"65557","type":"ColumnDataSource"},{"attributes":{"overlay":{"id":"65546"}},"id":"65540","type":"BoxZoomTool"},{"attributes":{"formatter":{"id":"65572"},"ticker":{"id":"65531"}},"id":"65530","type":"LinearAxis"},{"attributes":{"line_alpha":0.1,"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"65559","type":"Line"},{"attributes":{},"id":"65528","type":"LinearScale"},{"attributes":{"below":[{"id":"65530"}],"center":[{"id":"65533"},{"id":"65537"},{"id":"65579"}],"left":[{"id":"65534"}],"output_backend":"webgl","plot_height":500,"plot_width":500,"renderers":[{"id":"65560"},{"id":"65565"},{"id":"65584"}],"title":{"id":"65586"},"toolbar":{"id":"65548"},"toolbar_location":null,"x_range":{"id":"65522"},"x_scale":{"id":"65526"},"y_range":{"id":"65524"},"y_scale":{"id":"65528"}},"id":"65521","subtype":"Figure","type":"Plot"},{"attributes":{"toolbar":{"id":"65603"},"toolbar_location":"above"},"id":"65604","type":"ToolbarBox"},{"attributes":{"source":{"id":"65562"}},"id":"65566","type":"CDSView"},{"attributes":{},"id":"65531","type":"BasicTicker"},{"attributes":{},"id":"65570","type":"BasicTickFormatter"},{"attributes":{},"id":"65526","type":"LinearScale"},{"attributes":{"overlay":{"id":"65547"}},"id":"65542","type":"LassoSelectTool"},{"attributes":{"children":[{"id":"65604"},{"id":"65602"}]},"id":"65605","type":"Column"},{"attributes":{},"id":"65544","type":"SaveTool"},{"attributes":{"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"65558","type":"Line"},{"attributes":{"text":"y","text_font_size":{"value":"15pt"}},"id":"65586","type":"Title"},{"attributes":{},"id":"65538","type":"ResetTool"},{"attributes":{"toolbars":[{"id":"65548"}],"tools":[{"id":"65538"},{"id":"65539"},{"id":"65540"},{"id":"65541"},{"id":"65542"},{"id":"65543"},{"id":"65544"},{"id":"65545"}]},"id":"65603","type":"ProxyToolbar"},{"attributes":{},"id":"65522","type":"DataRange1d"},{"attributes":{},"id":"65541","type":"WheelZoomTool"},{"attributes":{"items":[{"id":"65580"}]},"id":"65579","type":"Legend"},{"attributes":{"data_source":{"id":"65562"},"glyph":{"id":"65563"},"hover_glyph":null,"muted_glyph":null,"nonselection_glyph":{"id":"65564"},"selection_glyph":null,"view":{"id":"65566"}},"id":"65565","type":"GlyphRenderer"},{"attributes":{},"id":"65576","type":"Selection"},{"attributes":{"fill_color":{"value":"#2a2eec"},"size":{"units":"screen","value":6.0},"x":{"value":-1.135840820153207},"y":{"value":0}},"id":"65582","type":"Circle"}],"root_ids":["65605"]},"title":"Bokeh Application","version":"2.2.3"}}';
                  var render_items = [{"docid":"d6d6ced0-0681-4e99-9a96-1bba85ce6f13","root_ids":["65605"],"roots":{"65605":"26914ec4-4351-491c-841e-d43858f81d35"}}];
                  root.Bokeh.embed.embed_items(docs_json, render_items);
                
                  }
                  if (root.Bokeh !== undefined) {
                    embed_document(root);
                  } else {
                    var attempts = 0;
                    var timer = setInterval(function(root) {
                      if (root.Bokeh !== undefined) {
                        clearInterval(timer);
                        embed_document(root);
                      } else {
                        attempts++;
                        if (attempts > 100) {
                          clearInterval(timer);
                          console.log("Bokeh: ERROR: Unable to run BokehJS code because BokehJS library is missing");
                        }
                      }
                    }, 10, root)
                  }
                })(window);
              });
            };
            if (document.readyState != "loading") fn();
            else document.addEventListener("DOMContentLoaded", fn);
          })();
        },
        function(Bokeh) {
        
        
        }
      ];
    
      function run_inline_js() {
        
        for (var i = 0; i < inline_js.length; i++) {
          inline_js[i].call(root, root.Bokeh);
        }
        
      }
    
      if (root._bokeh_is_loading === 0) {
        console.debug("Bokeh: BokehJS loaded, going straight to plotting");
        run_inline_js();
      } else {
        load_libs(css_urls, js_urls, function() {
          console.debug("Bokeh: BokehJS plotting callback run at", now());
          run_inline_js();
        });
      }
    }(window));
  };
  if (document.readyState != "loading") fn();
  else document.addEventListener("DOMContentLoaded", fn);
})();